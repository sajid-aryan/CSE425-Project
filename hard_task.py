...
from src.dataset import AudioDataset, LyricsDataset, HybridDataset
from src.vae import BetaVAE, ConditionalVAE, train_vae
...
def train_beta_vae(model, train_loader, optimizer, device, epochs=100, beta=4.0):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, _) in enumerate(train_loader):
            ...
            # Forward pass
            output = model(data)
            recon_loss = F.mse_loss(output['reconstruction'], data, reduction='mean')
...
class Autoencoder(nn.Module):
    """Simple deterministic autoencoder baseline (no KL term)."""
    ...
    def forward(self, x: torch.Tensor):
        ...
        return {'reconstruction': recon, 'z': z}


def train_autoencoder(model, train_loader, optimizer, device, epochs=50):
    model.train()
    losses = []
    for epoch in range(epochs):
        total = 0.0
        for data, _ in train_loader:
            ...
            optimizer.zero_grad()
            out = model(data)
            loss = F.mse_loss(out['reconstruction'], data, reduction='mean')
            if not torch.isfinite(loss):
                continue
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
            optimizer.step()
            total += loss.item()
        avg = total / max(1, len(train_loader.dataset))
        losses.append(avg)
        if epoch % 20 == 0 or epoch == epochs - 1:
            print(f"AE Epoch [{epoch}/{epochs}], Loss: {avg:.4f}")
...
def run_hard_task():
    ...
    # NOTE: Feature extraction is CPU-bound; caching prevents re-extracting per epoch.
    ...
    # Load lyrics dataset (English + Bangla)
    lyrics_files = []
    ...
    # Reconstruction examples (feature-space) for Beta-VAE
    print("\n🖼️  Saving reconstruction examples...")
    os.makedirs("results", exist_ok=True)
    beta_vae.eval()
    with torch.no_grad():
        batch_x, _ = next(iter(test_loader))
        batch_x = batch_x.to(device)
        out = beta_vae(batch_x)
        x_np = batch_x[:6].detach().cpu().numpy()
        r_np = out['reconstruction'][:6].detach().cpu().numpy()

    fig, axes = plt.subplots(6, 1, figsize=(12, 10), sharex=True)
    for i in range(6):
        axes[i].plot(x_np[i], label='original', linewidth=1)
        axes[i].plot(r_np[i], label='recon', linewidth=1, alpha=0.8)
        axes[i].set_ylabel(f"s{i}")
        if i == 0:
            axes[i].legend(loc='upper right', ncol=2)
    axes[-1].set_xlabel('feature index')
    fig.suptitle('Beta-VAE reconstruction examples (feature space)')
    plt.tight_layout()
    plt.savefig("results/hard_task_reconstructions.png", dpi=300, bbox_inches='tight')
    plt.close(fig)
    ...
    print("🔄 Training Autoencoder baseline for comparison...")
    autoencoder = Autoencoder(input_dim=input_dim, hidden_dim=512, latent_dim=64).to(device)
    ae_optimizer = optim.Adam(autoencoder.parameters(), lr=1e-3)
    ae_history = train_autoencoder(autoencoder, train_loader, ae_optimizer, device, epochs=50)
    ...
    with torch.no_grad():
        for data, _ in test_loader:
            ...
            ae_latents.append(out['z'].detach().cpu().numpy())
    ae_latent_features = np.vstack(ae_latents)
    ae_clustering = ClusteringPipeline(n_clusters=n_genres)
    ae_cluster_results = ae_clustering.fit_transform(ae_latent_features)
    ...
    # Multimodal fusion: concatenate audio latent + reduced lyrics embedding
    beta_hybrid_features = None
    regular_hybrid_features = None
    ae_hybrid_features = None
    if lyrics_dataset is not None and len(lyrics_dataset) > 0:
        vocab_size = 1000
        lyrics_embeds = np.zeros((len(true_labels), vocab_size), dtype=np.float32)
        tmp_hybrid = HybridDataset(audio_dataset, lyrics_dataset)
        for i in range(len(true_labels)):
            text, _lang = lyrics_dataset[i % len(lyrics_dataset)]
            lyrics_embeds[i] = tmp_hybrid.encode_text(text).numpy().astype(np.float32, copy=False)

        from sklearn.decomposition import PCA
        n_comp = min(32, lyrics_embeds.shape[0], lyrics_embeds.shape[1])
        lyrics_reduced = PCA(n_components=n_comp, random_state=42).fit_transform(lyrics_embeds)

        beta_hybrid_features = np.concatenate([beta_latent_features, lyrics_reduced], axis=1)
        regular_hybrid_features = np.concatenate([regular_latent_features, lyrics_reduced], axis=1)
        ae_hybrid_features = np.concatenate([ae_latent_features, lyrics_reduced], axis=1)
    ...
    all_cluster_results = {
        ...
        'autoencoder_kmeans': ae_cluster_results['kmeans'],
        'autoencoder_agglomerative': ae_cluster_results['agglomerative'],
    }

    # Hybrid clustering methods
    if beta_hybrid_features is not None:
        hybrid_clusterer = ClusteringPipeline(n_clusters=n_genres)
        all_cluster_results['beta_vae_hybrid_kmeans'] = hybrid_clusterer.fit_transform(beta_hybrid_features)['kmeans']
        all_cluster_results['regular_vae_hybrid_kmeans'] = hybrid_clusterer.fit_transform(regular_hybrid_features)['kmeans']
        all_cluster_results['autoencoder_hybrid_kmeans'] = hybrid_clusterer.fit_transform(ae_hybrid_features)['kmeans']
    ...
    for method_name, labels in all_cluster_results.items():
        ...
        if 'beta_vae' in method_name:
            features_for_eval = beta_hybrid_features if ('hybrid' in method_name and beta_hybrid_features is not None) else beta_latent_features
        elif 'regular_vae' in method_name:
            features_for_eval = regular_hybrid_features if ('hybrid' in method_name and regular_hybrid_features is not None) else regular_latent_features
        elif 'autoencoder' in method_name:
            features_for_eval = ae_hybrid_features if ('hybrid' in method_name and ae_hybrid_features is not None) else ae_latent_features
        else:
